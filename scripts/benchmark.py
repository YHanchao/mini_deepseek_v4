...
CONFIGS = {
    ...
    "medium": dict(
        ...
        max_seq_len=2048, compress_ratios=(0, 0, 4, 128, 0, 0, 4, 128, 0, 0, 4, 128, 0, 0),
...
def benchmark_config(name, cfg_overrides, batch_size=4, seq_len=128,
                     warmup_steps=5, profile_steps=15):
    print(f"\n{'='*65}")
    ...
    args = DSArgs(**{k: cfg_overrides[k] for k in cfg_overrides
                     if k in base})
...
def main():
    ...
    # Phase 1: fixed seq_len=128, compare configs
    print("\n\n--- Phase 1: seq_len=128, batch_size=4 ---")
    for name in ["tiny", "small", "medium"]:
        r = benchmark_config(name, CONFIGS[name], batch_size=4, seq_len=128)
        if r:
            results.append(r)
    ...
    for bs in [4, 2, 1]:
        r = benchmark_config("default", CONFIGS["default"], batch_size=bs, seq_len=128,
                            warmup_steps=3, profile_steps=10)
        if r:
            results.append(r)
    ...
    print("SUMMARY")
    print("=" * 65)
    print(f"{'Config':<12s} {'Params':>10s} {'Batch':>6s} {'Seq':>5s} "
          f"{'tok/s':>10s} {'Step(ms)':>9s} {'PeakMem':>8s}")
    print("-" * 65)
    for r in results:
        print(f"{r['name']:<12s} {format_params(r['params']):>10s} "
    ...
    for r in results:
        if r['tok_per_sec'] > 1000:
            ...
            print(f"  {r['name']}: ~{steps_per_hour:,.0f} steps/hour "
